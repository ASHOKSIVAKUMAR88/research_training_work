...
{
    int semid = semget(atoi(argv[1]), 0, IPC_CREAT|IPC_EXCL|S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH);
    if(semid == -1)
    {
        perror("ERROR");
        semid = semget(atoi(argv[1]),0, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH);
    }
